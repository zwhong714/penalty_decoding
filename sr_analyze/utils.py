...
class GenerationMixin:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        '''
        Add previous neuclues size 
        '''
        cur_len = 0
        sum_nucleus = torch.zeros(input_ids.shape[0], 5).to('cuda')
        sr = torch.zeros(input_ids.shape[0], 5).to('cuda')

        '''
        ADD SR_token, SR_phrase
        '''
        initial_probability_token = torch.zeros(input_ids.shape[0], 50257).to('cuda')
        is_exist = torch.zeros(input_ids.shape[0], 50257).to('cuda')
        is_exist = is_exist.scatter(1, input_ids, 1)
        initial_probability_token =initial_probability_token.scatter(1, input_ids, 1.0)
        total_repetition_token = torch.zeros(input_ids.shape[0], 1).to('cuda')
        strengthening_count = torch.zeros(input_ids.shape[0], 1).to('cuda')

        def _get_ngrams(ngram_size: int, prev_input_ids: torch.Tensor, num_hypos: int):
            generated_ngrams = [{} for _ in range(num_hypos)]
            initial_probability_phrases = [{} for _ in range(num_hypos)]

            for idx in range(num_hypos):
                gen_tokens = prev_input_ids[idx].tolist()
                generated_ngram = generated_ngrams[idx]
                initial_probability_phrase = initial_probability_phrases[idx]
                for ngram in zip(*[gen_tokens[i:] for i in range(ngram_size)]):
                    prev_ngram_tuple = tuple(ngram)
                    generated_ngram[prev_ngram_tuple] = generated_ngram.get(prev_ngram_tuple, 0) + 1
                    initial_probability_phrase[prev_ngram_tuple] = initial_probability_phrase.get(prev_ngram_tuple, 0.0) + 1e-9
            return generated_ngrams, initial_probability_phrases

        generated_2grams, initial_probability_2gram = _get_ngrams(2, input_ids, input_ids.shape[0])
        generated_3grams, initial_probability_3gram = _get_ngrams(3, input_ids, input_ids.shape[0])
        generated_4grams, initial_probability_4gram = _get_ngrams(4, input_ids, input_ids.shape[0])

        total_repetition_2grams = torch.zeros(input_ids.shape[0], 1).to('cuda')
        strengthening_count_2grams = torch.zeros(input_ids.shape[0], 1).to('cuda')
        total_repetition_3grams = torch.zeros(input_ids.shape[0], 1).to('cuda')
        strengthening_count_3grams = torch.zeros(input_ids.shape[0], 1).to('cuda')
        total_repetition_4grams = torch.zeros(input_ids.shape[0], 1).to('cuda')
        strengthening_count_4grams = torch.zeros(input_ids.shape[0], 1).to('cuda')
        '''
        Done!
        '''
        ...
        while True:
            ...
            '''
            ADD! get top_k neucles_size
            '''
            probs = nn.functional.softmax(next_tokens_scores, dim=-1)
            cur_len += 1

            # BUG: just one sentence
            indices_to_left = probs >= torch.topk(probs, 2)[0][..., -1, None]
            p2 = torch.sum(probs[indices_to_left])
            if cur_len - 1 != 0:
                if (sum_nucleus[0][0] / (cur_len - 1) ) < ((sum_nucleus[0][0] + p2) / cur_len):
                    sr[0][0] += 1
            sum_nucleus[0][0] += p2
           
            indices_to_left = probs >= torch.topk(probs, 4)[0][..., -1, None]
            p4 = torch.sum(probs[indices_to_left])
            if cur_len - 1 != 0:
                if (sum_nucleus[0][1] / (cur_len - 1) ) < ((sum_nucleus[0][1] + p4) / cur_len):
                    sr[0][1] += 1
            sum_nucleus[0][1] += p4

            indices_to_left = probs >= torch.topk(probs, 6)[0][..., -1, None]
            p6 = torch.sum(probs[indices_to_left])
            if cur_len - 1 != 0:
                if (sum_nucleus[0][2] / (cur_len - 1) ) < ((sum_nucleus[0][2] + p6) / cur_len):
                    sr[0][2] += 1
            sum_nucleus[0][2] += p6

            indices_to_left = probs >= torch.topk(probs, 8)[0][..., -1, None]
            p8 = torch.sum(probs[indices_to_left])
            if cur_len - 1 != 0:
                if (sum_nucleus[0][3] / (cur_len - 1) ) < ((sum_nucleus[0][3] + p8) / cur_len):
                    sr[0][3] += 1
            sum_nucleus[0][3] += p8

            indices_to_left = probs >= torch.topk(probs, 10)[0][..., -1, None]
            p10 = torch.sum(probs[indices_to_left])
            if cur_len - 1 != 0:
                if (sum_nucleus[0][4] / (cur_len - 1) ) < ((sum_nucleus[0][4] + p10) / cur_len):
                    sr[0][4] += 1
            sum_nucleus[0][4] += p10
            ...
            # argmax
            # probs = nn.functional.softmax(next_tokens_scores, dim=-1)
            next_probs, next_tokens = torch.max(probs, dim=-1) 
            ...
            for i in range(input_ids.shape[0]):
                ...
                if is_exist[i][id]:
                    total_repetition_token[i] += 1
                    if initial_probability_token[i][id] < next_probs[i]:
                        strengthening_count[i] += 1
                is_exist[i][id] = 1
                initial_probability_token[i][id] = next_probs[i]

                if input_ids.shape[1] > 2:
                    cur = (input_ids[i][-1].item(), id.item(), )
                    mean_prob = (initial_probability_token[i][input_ids[i][-1]] + next_probs[i]) / 2
                    if generated_2grams[i].get(cur, 0):
                        total_repetition_2grams[i] += 1
                        if initial_probability_2gram[i][cur] < mean_prob:
                            strengthening_count_2grams[i] += 1
                    
                    generated_2grams[i][cur] = 1
                    initial_probability_2gram[i][cur] = mean_prob
                        
                
                if input_ids.shape[1] > 3:
                    cur = (input_ids[i][-2].item(), input_ids[i][-1].item(), id.item(), )
                    mean_prob = (initial_probability_token[i][input_ids[i][-2]]  + 
                                        initial_probability_token[i][input_ids[i][-1]] + next_probs[i]) / 3
                    if generated_3grams[i].get(cur, 0):
                        total_repetition_3grams[i] += 1
                        
                        if initial_probability_3gram[i][cur] < mean_prob:
                            strengthening_count_3grams[i] += 1
                    
                    generated_3grams[i][cur] = 1
                    initial_probability_3gram[i][cur] = mean_prob

                
                if input_ids.shape[1] > 4:
                    cur = (input_ids[i][-3].item(), input_ids[i][-2].item(), input_ids[i][-1].item(), id.item(), )
                    mean_prob = (initial_probability_token[i][input_ids[i][-3]] + initial_probability_token[i][input_ids[i][-2]]  + 
                                        initial_probability_token[i][input_ids[i][-1]] + next_probs[i]) / 4
                    if generated_4grams[i].get(cur, 0):
                        total_repetition_4grams[i] += 1
                        if initial_probability_4gram[i][cur] < mean_prob:
                            strengthening_count_4grams[i] += 1
                    
                    generated_4grams[i][cur] = 1
                    initial_probability_4gram[i][cur] = mean_prob
            """
            Done!
            """
            # finished sentences should have their next token be a padding token
        ...
        if return_dict_in_generate:
            ...
            modify!
            '''
            return input_ids, torch.mean(strengthening_count / cur_len), torch.mean(strengthening_count_2grams / max(1, cur_len - 1)), torch.mean(strengthening_count_3grams / max(1, cur_len - 2)), torch.mean(strengthening_count_4grams / max(1, cur_len - 4)), sr / cur_len, [p2, p4, p6, p8, p10]
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        def _get_ngrams(ngram_size: int, prev_input_ids: torch.Tensor, num_hypos: int):
            ...
            for idx in range(num_hypos):
                ...
                for ngram in zip(*[gen_tokens[i:] for i in range(ngram_size)]):
                    prev_ngram_tuple = tuple(ngram)
                    generated_ngram[prev_ngram_tuple] = generated_ngram.get(prev_ngram_tuple, 0) + 1
                    initial_probability_phrase[prev_ngram_tuple] = initial_probability_phrase.get(prev_ngram_tuple, 1.0)
            return generated_ngrams, initial_probability_phrases
        ...
        while True:
            ...
            #origin_probs = nn.functional.softmax(next_token_logits, dim=-1)
            next_probs = torch.gather(probs, 1, next_tokens.unsqueeze(1)).squeeze(1)
